...
AgniDrishti Frontend

Open Dashboard

AgniDrishti Backend Health Check

Restart / Wake Backend
...
Return to AgniDrishti Dashboard and refresh.
...
Non-fire / background observation
...
Backend /health — wake/check deployed server
...
Satellite / Earth-observation data provides the underlying observations.

Relevant features are extracted and processed.

The ML classifier evaluates observations for fire-related patterns.

Classified observations can be represented as events.

Geographic information is stored using PostgreSQL + PostGIS.

The FastAPI backend exposes the resulting information through APIs.

The Next.js frontend requests the data.

The dashboard presents the events geographically and visually for monitoring and analysis.
...
🔥 Open AgniDrishti Dashboard

❤️ Backend Health / Wake Server
